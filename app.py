...
def prediccion():
    try:
        ...
        # Ventaja de jugar en casa (factor 1.15)
        ...
        if pct_empate < 5:
            pct_empate = 10
            if pct_local > pct_visita:
                pct_local -= 5
            else:
                pct_visita -= 5

        # Marcador probable basado en goles por partido
        ...
        if pct_local > pct_visita and pct_local > pct_empate:
            ...
            resultado = f"{visita} gana"
            emoji = "🏆"
        else:
            resultado = "Empate probable"
            emoji = "🤝"
        ...
        return jsonify({
            ...
            "marcador": f"{marc_local}-{marc_visita}",
            "razon": f"{local} (pos.{pos_local}) vs {visita} (pos.{pos_visita}). Ventaja local considerada."
        })
    except Exception as e:
